...
public class NumericBlockElementBeanView extends LinearLayout {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (enableHighlighterColor) {
			NumericBlockRenderer.renderNumericBlockHighlighter(canvas, path, measuredWidth, measuredHeight);
		} else if (mNumericBlockElementBean.getAllowInput()) {
			NumericBlockRenderer.renderNumericBlockElement(canvas, path, measuredWidth, measuredHeight);
		} else {
			NumericBlockRenderer.renderNonInputableNumericBlockElement(canvas, path, measuredWidth, measuredHeight);
		}
	}
}
