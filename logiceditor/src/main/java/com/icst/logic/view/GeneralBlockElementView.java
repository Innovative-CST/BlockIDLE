	...
	protected void onDraw(Canvas canvas) {
		if (enableHighlighterColor) {
			...
			if (!blockAttached) {
				...
				renderBlockBackdrop(currentBlockBean, canvas);
			}
		}
	}

	private void renderBlockBackdrop(ExpressionBlockBean currentBlockBean, Canvas canvas) {
		...
		if (currentBlockBean instanceof StringBlockBean stringBlockBean) {
			...
			BlockShapesUtils.drawBooleanBlock(canvas, getContext(), measuredWidth, measuredHeight, Color.BLACK);
		} else if (currentBlockBean instanceof NumericBlockBean numericBlockBean) {
			NumericBlockRenderer.renderNumericBlockBackdrop(canvas, path, measuredWidth, measuredHeight);
		} else if (currentBlockBean instanceof GeneralExpressionBlockBean generalExpressionBlockBean) {
			BlockShapesUtils.drawGeneralBlockElement(canvas, getContext(), measuredWidth, measuredHeight, Color.BLACK);
		}
	}
