...
import com.icst.blockidle.terminal.VirtualKeysConstants.EXTRA_KEY_DISPLAY_MAPS;
 ...
 *
 * <p>
 * The {@code propertiesInfo} passed to the constructors of this class must be
 * json array of
 * arrays. Each array element of the json array will be considered a separate
 * row of keys. Each key
 * can either be simple string that defines the name of the key or a json dict
 * that defines advance
 * info for the key. The syntax can be `'KEY'` or `{key: 'KEY'}`. For example
 * `HOME` or `{key:
 * 'HOME', ...}.
 *
 * <p>
 * In advance json dict mode, the key can also be a sequence of space separated
 * keys instead of
 * one key. This can be done by replacing `key` key/value pair of the dict with
 * a `macro` key/value
 * pair. The syntax is `{macro: 'KEY COMBINATION'}`. For example {macro: 'HOME
 * RIGHT', ...}.
 *
 * <p>
 * In advance json dict mode, you can define a nested json dict with the `popup`
 * key which will
 * be used as the popup key and will be triggered on swipe up. The syntax can be
 * `{key: 'KEY',
 * popup: 'POPUP_KEY'}` or `{key: 'KEY', popup: {macro: 'KEY COMBINATION',
 * display: 'Key combo'}}`.
 * For example `{key: 'HOME', popup: {KEY: 'END', ...}, ...}`.
 *
 * <p>
 * In advance json dict mode, the key can also have a custom display name that
 * can be used as the
 * text to display on the button by defining the `display` key. The syntax is
 * `{display:
 ...
 * <p>
 * #
 * Advance: [[ {key: ESC, popup: {macro: "CTRL f d", display: "tmux exit"}},
 * {key: CTRL, popup:
 * {macro: "CTRL f BKSP", display: "tmux ←"}}, {key: ALT, popup: {macro: "CTRL f
 * TAB", display:
 * "tmux →"}}, {key: TAB, popup: {macro: "ALT a", display: A-a}}, {key: LEFT,
 * popup: HOME}, {key:
 * DOWN, popup: PGDN}, {key: UP, popup: PGUP}, {key: RIGHT, popup: END}, {macro:
 * "ALT j", display:
 * A-j, popup: {macro: "ALT g", display: A-g}}, {key: KEYBOARD, popup: {macro:
 * "CTRL d", display:
 * exit}} ]]
 *
 * <p>
 ...
 *
 * <p>
 * Aliases are also allowed for the keys that you can pass as
 * {@code extraKeyAliasMap}. Check
 * {@link VirtualKeysConstants#CONTROL_CHARS_ALIASES}.
 *
 * <p>
 * Its up to the {@link VirtualKeysView.IVirtualKeysView} client on how to
 * handle individual key
 * values of an {@link VirtualKeyButton}. They are sent as is via {@link
 * VirtualKeysView.IVirtualKeysView#onVirtualKeyButtonClick(View, VirtualKeyButton, Button)}.
 * The
 * {@link com.termux.shared.terminal.io.TerminalVirtualKeys} which is an
 * implementation of the
 * interface, checks if the key is one of
 * {@link VirtualKeysConstants#PRIMARY_KEY_CODES_FOR_STRINGS}
 * and generates a {@link android.view.KeyEvent} for it, and if its not, then
 * converts the key to
 * code points by calling {@link CharSequence#codePoints()} and passes them to
 * the terminal as
 * literal strings.
 *
 * <p>
 * Examples:
 * {@code "ENTER" will trigger the ENTER keycode "LEFT" will trigger the LEFT keycode
...
public class VirtualKeysInfo {
	 ...
	 * @param style
	 *            The style to pass to {@link #getCharDisplayMapForStyle(String)} to
	 *            get the {@link
	 *            VirtualKeysConstants.VirtualKeyDisplayMap} that defines the
	 *            display text mapping for the
	 *            keys if a custom value is not defined by
	 *            {@link VirtualKeyButton#KEY_DISPLAY_NAME} for a
	 *            key.
	...
	@NonNull public static VirtualKeysConstants.VirtualKeyDisplayMap getCharDisplayMapForStyle(String style) {
		switch (style) {
			case "arrows-only":
	 ...
	 *            The {@link String} containing the info to create the {@link
	 *            VirtualKeysInfo}. Check the class javadoc for details.
	 * @param extraKeyDisplayMap
	 *            The {@link VirtualKeysConstants.VirtualKeyDisplayMap} that defines
	 *            the display text mapping for the keys if a custom value is not
	 *            defined by {@link
	 *            VirtualKeyButton#KEY_DISPLAY_NAME} for a key. You can create your
	 *            own or optionally pass
