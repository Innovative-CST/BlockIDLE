...
public final class ControlBlocks {
	...
	public static BlockPaletteBean getControlBlockPalette() {
		...
		blocks.add(tryCatchFinallyBlock());
	...
	private static BlockBean tryCatchFinallyBlock() {
		RegularBlockBean block = new RegularBlockBean();
		block.setColor("#FFC041");
		...
		LabelBlockElementBean tryLabel = new LabelBlockElementBean();
		tryLabel.setLabel("try");
		layer1Elements.add(tryLabel);
		...
		LabelBlockElementBean catchELabel = new LabelBlockElementBean();
		catchELabel.setLabel("catch Exception e");
		layer3Elements.add(catchELabel);

		layer3.setBlockElementBeans(layer3Elements);

		layers.add(layer3);

		ActionBlockLayerBean catchStatementLayer = new ActionBlockLayerBean();
		catchStatementLayer.setKey("catchStatement");

		layers.add(catchStatementLayer);

		BlockElementLayerBean layer5 = new BlockElementLayerBean();

		ArrayList<BlockElementBean> layer5Elements = new ArrayList<BlockElementBean>();

		LabelBlockElementBean finallyLabel = new LabelBlockElementBean();
		finallyLabel.setLabel("finally");
		layer5Elements.add(finallyLabel);

		layer5.setBlockElementBeans(layer5Elements);

		layers.add(layer5);

		ActionBlockLayerBean finallyStatementLayer = new ActionBlockLayerBean();
		finallyStatementLayer.setKey("finallyStatement");

		layers.add(finallyStatementLayer);
		...
		code.append(CodeFormatterUtils.getKeySyntaxString("statements"));
		code.append("\n} catch (Exception e) {\n\t");
		code.append(CodeFormatterUtils.getKeySyntaxString("catchStatement"));
		code.append("\n} finally {\n\t");
		code.append(CodeFormatterUtils.getKeySyntaxString("finallyStatement"));
		code.append("\n}");
