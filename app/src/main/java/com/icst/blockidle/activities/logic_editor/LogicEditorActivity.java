...
public class LogicEditorActivity extends BaseActivity {
	...
	public static final String LANGUAGE_MODE_ARGUMENT = "LanguageMode";
	...
	private ProgrammingLanguage getLanguageMode() {
		return ProgrammingLanguage.fromString(getIntent().getStringExtra(LANGUAGE_MODE_ARGUMENT));
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mBlockPaletteBean.removeIf(palette -> {
			...
			palette.getBlocks().removeIf(block -> {
				BeanManifest beanManifest = block.getBeanManifest();
				if (beanManifest == null)
					return false;
				ArrayList<SupportedLanguagesBean> supportedLanguages = beanManifest.get(SupportedLanguagesBean.class);
				for (SupportedLanguagesBean supportedLanguagesBean : supportedLanguages) {
					if (supportedLanguagesBean.isLanguageSupported(getLanguageMode())) {
						return false;
					}
				}
